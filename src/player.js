class Player {
  ...
  static setPuyoPosition() {
    ...
    if (y + 1 >= Config.stageRows || Stage.board[y + 1][x] || (y + dy + 1 >= 0 && (y + dy >= Config.stageRows || Stage.board[y + dy + 1][x + dx]))) {
      isBlocked = true;
    }
    if (!isBlocked) {
      ...
      if (Math.floor(this.puyoStatus.top / Config.puyoImgHeight) != y) {
        ...
        if (!isBlocked) {
          //境を超えたが特に問題はなかった。次回もジユ落下を続ける
          this.groundFrame = 0;
          return;
        } else {
          // 境を超えたらブロックにぶつかった。位置を調節して、設置を開始する
    ...
    if (this.keyStatus.right || this.keyStatus.left) {
      ...
      if (my < 0 || mx + cx || mx + cx Config.stageCols || Stage.board[y][x + cx]) {
        if (my >= 0) {
          canMove = false;
        }
      }
      //設置していない場合は、更に一個下のブロックの左右も確認する
      if (this.groundFrame == 0) {
      ...
      // 回転を確認する
      // 合わせるかどうかは後で確認する。まわすぞ
      const x = this.puyoStatus.x;
      const y = this.puyoStatus.y;
      const mx = x + this.puyoStatus.dx;
      const my = y + this.puyoStatus.dy;
      const rotasion = this.puyoStatus.rotation;
      let canRotate = true;

      let cx = 0;
      let cy = 0;
      if (totation === 0) {
        //右から上には100%確実に回せるので何もしない
      } else if (rotation === 90) {
        //上から左に回すときに左にブロックがあれば右に移動する必要があるのでまず確認する
      ...
      if (canRotate) {
        ...
        //回す事ができるので、回転後の情報をセットして回転状態にする
        this.actionStartFrame = frame;
        this.rotateBeforeLeft = x * Config.puyoImgHeight;
        this.rotateAfterLeft = (x + cx) * Config.puyoImgHeight;
        this.rotateFromRotate = this.puyoStatus.rotation;
        //次の状態を先に設定しておく
        this.puyoStatus.x += cx;
        const distRotation = (this.puyoStatus.rotasion + 90) % 360;
